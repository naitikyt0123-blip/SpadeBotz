  ...
  try {
    ...
    try {
      const emailInput = page
        .locator("input[type='email'], input[placeholder*='email'], input[placeholder*='@']")
        .first();
    ...
    let loggedIn =
      ...
      finalContent.includes("fund");

    if (loggedIn) {
      await sendMsg(`✅ LOGIN SUCCESS! Sach me login ho gaya.\n🔗 URL: ${currentUrl}`);
    } else {
      await sendMsg(`⚠️ Login confirm nahi hua. Code galat ho sakta hai ya extra step.\n🔗 URL: ${currentUrl}`);
    }

    // Screenshot proof
    try {
      const shot = await page.screenshot({ fullPage: true });
      await bot.sendPhoto(CHAT_ID, shot, { caption: "📸 Final screenshot (proof)" });
    } catch (e) {
